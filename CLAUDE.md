...
- 2026-09-24: 재고 시스템에서 바로 뽑은 원본 xlsx가 업로드 시 "재고현황 파일로 보이지 않습니다"로 실패하는 버그. 원인은 그런 원본 파일의 시트 dimension 정보가 부실해 openpyxl `read_only`(스트리밍) 모드가 헤더 1행만 읽고 멈춰 데이터가 0행이 되는 것(실측: 유원 4000행 파일이 read_only에서 1행). **엑셀로 열어 다시 저장한 파일은 정상 xlsx로 바뀌어 잘 읽혀서**, 사용자가 헤더 확인용으로 준 샘플로는 재현이 안 됐다(브라우저/전송/캐시 문제로 오인). `inventory.parse_inventory_xlsx`가 `read_only=True`로 0행이면 `read_only=False`(전체 로드)로 자동 재시도하도록 수정(유원 파일 0→3995행 확인). 폴백 로직 pytest 추가. 진단은 임시로 에러 메시지에 받은 바이트 정보를 붙여 잡았고 원인 확인 후 원복했다.
