...
- `/admin`을 탭 3개로 나눴다(`data-tab` + `showAdminTab()`): "보물찾기 운영"(보물 심기/심은 보물/검토 대기/기본 포인트·통계/랭킹보드), "매장 마스터"(엑셀 마스터 업로드/판매점 빠른 등록/판매점 목록), "계정 · 공통"(SKT 직원 계정/재고 엑셀 안내/내 비밀번호/대리점/영업사원). 기본 탭은 "운영". 역할별 숨김(`data-super-only`, `.hidden`)과 탭별 숨김(`.tab-hidden`)은 서로 다른 클래스라 독립적으로 동작한다 — 하나를 토글해도 다른 쪽 상태를 안 건드림.
